...
export const apiFetch = async (path, { method = "GET", body, headers = {}, auth = true } = {}) => {
  ...
  if (!response.ok) {
    ...
    if (response.status === 401 && auth) {
      // A protected request can fail after the initial app bootstrap (for
      // example when a JWT expires while the dashboard is open). Let the
      // route guard end the session consistently instead of leaving the user
      // on a page full of failed API calls.
      window.dispatchEvent(new CustomEvent("eps:session-expired"));
    }
    throw err;
  }
