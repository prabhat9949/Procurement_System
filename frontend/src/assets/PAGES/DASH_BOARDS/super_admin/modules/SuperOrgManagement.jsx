...
import { Globe, Building2, Users, Landmark, Briefcase, UserCheck } from "lucide-react";
...
const SuperOrgManagement = () => (
  <LiveRoleOverview
    header={{
      ...
      subtitle: "Departments, cost centers and employees — real master data from the database.",
      badge: "ORGANIZATION",
      icon: Globe,
      accent: "#f8b400",
    }}
    ...
    kpiFn={(data) => {
      ...
      return [
        ...
        { label: "Active Employees", value: emps.filter((e) => e.active).length, icon: UserCheck, color: "#7c3aed" },
        { label: "Budgeted Cost Centers", value: ccs.filter((c) => Number(c.budget || 0) > 0).length, icon: Briefcase, color: "#0891b2" },
      ];
    }}
    tables={[
      {
      ...
      },
      {
        ...
        maxRows: 8,
        columns: [
          { header: "Emp Code", render: (r) => <strong style={{ color: "#7c3aed" }}>{r.employeeCode}</strong> },
          { header: "Name", render: (r) => <span style={{ fontWeight: 700 }}>{`${r.firstName} ${r.lastName}`}</span> },
          { header: "Department", accessor: "departmentName" },
