...
const ExecSettings = () => {
  ...
  const switchRow = (label, desc, key) => (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 0", borderBottom: "1px solid #ececec" }}>
      ...
      <button
        type="button"
        onClick={() => toggle(key)}
        style={{ width: "44px", height: "24px", borderRadius: "12px", border: "none", background: prefs?.[key] ? "#059669" : "#d9d9d9", position: "relative", cursor: "pointer", transition: "background 0.2s" }}
      >
        <span style={{ position: "absolute", top: "3px", left: prefs?.[key] ? "24px" : "3px", width: "18px", height: "18px", borderRadius: "50%", background: "#fff", transition: "left 0.2s" }} />
      </button>
    </div>
  );
  ...
  const card = { background: "#fff", borderRadius: "14px", border: "1px solid #ececec", padding: "24px" };

  return (
    <div className="pe-settings-container" style={{ padding: "20px" }}>
      {toastMsg && (
      ...
      <div className="pe-page-header" style={{ marginBottom: "24px" }}>
        <h1 className="pe-page-title" style={{ display: "flex", alignItems: "center", gap: "10px", fontSize: "24px", fontWeight: "800", color: "#111111" }}>
          <Settings color="#f8b400" size={28} /> Settings & Preferences
        </h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px" }}>
        <div style={card}>
          ...
          <p style={{ fontSize: "12.5px", color: "#888", marginBottom: "12px" }}>Control how workflow notifications reach you.</p>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "18px" }}>
            <button className="pe-btn-primary-sm" onClick={savePrefs} disabled={saving}>
          ...
          {canChangePassword ? (
            <form onSubmit={changePassword} style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
              <div className="pe-form-group">
                ...
                <input type="password" className="pe-form-input" value={pw.currentPassword} onChange={(e) => setPw({ ...pw, currentPassword: e.target.value })} required />
              </div>
              <div className="pe-form-group">
                <label className="pe-form-label">New Password *</label>
                <input type="password" className="pe-form-input" value={pw.newPassword} onChange={(e) => setPw({ ...pw, newPassword: e.target.value })} required />
              </div>
              <div className="pe-form-group">
                <label className="pe-form-label">Confirm New Password *</label>
                <input type="password" className="pe-form-input" value={pw.confirmPassword} onChange={(e) => setPw({ ...pw, confirmPassword: e.target.value })} required />
              </div>
