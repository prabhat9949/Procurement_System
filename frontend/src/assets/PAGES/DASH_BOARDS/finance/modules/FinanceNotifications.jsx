...
import { Bell, CheckCircle2, Loader2, WifiOff, Landmark } from "lucide-react";
    ...
    <div style={{ padding: "20px" }}>
      <div className="fin-page-header">
        <div>
          ...
          <p className="fin-page-subtitle">Invoice, matching and payment events — live from the database.</p>
        </div>
        <button className="fin-btn-primary-sm" onClick={markAllRead} disabled={!notifs.some((n) => !n.isRead)}>
      ...
      {loading ? (
        ...
        <div className="fin-card" style={{ textAlign: "center", padding: "48px" }}>
          <Landmark size={48} color="#059669" style={{ margin: "0 auto 12px" }} />
      ...
      {totalPages > 1 && (
        <div style={{ display: "flex", justifyContent: "center", gap: "10px", marginTop: "20px" }}>
          <button className="fin-btn-primary-sm" style={{ background: "#f8f9fb", color: "#111", border: "1px solid #d9d9d9" }} disabled={page === 0} onClick={() => setPage(page - 1)}>Previous</button>
          <span style={{ alignSelf: "center", fontSize: "13px", color: "#666", fontWeight: 600 }}>Page {page + 1} of {totalPages}</span>
          <button className="fin-btn-primary-sm" disabled={page + 1 >= totalPages} onClick={() => setPage(page + 1)}>Next</button>
        </div>
      )}
    </div>
  );
