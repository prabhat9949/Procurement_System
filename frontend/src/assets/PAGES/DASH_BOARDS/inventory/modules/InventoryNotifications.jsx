...
import { Bell, CheckCircle2, Loader2, WifiOff, Warehouse } from "lucide-react";
    ...
    <div style={{ padding: "20px" }}>
      <div className="inv-page-header">
        <div>
          ...
          <p className="inv-page-subtitle">Delivery, GRN and stock events — live from the database.</p>
        </div>
        <button className="inv-btn-primary-sm" onClick={markAllRead} disabled={!notifs.some((n) => !n.isRead)}>
      ...
      {totalPages > 1 && (
        <div style={{ display: "flex", justifyContent: "center", gap: "10px", marginTop: "20px" }}>
          <button className="inv-btn-primary-sm" style={{ background: "#f8f9fb", color: "#111", border: "1px solid #d9d9d9" }} disabled={page === 0} onClick={() => setPage(page - 1)}>Previous</button>
          <span style={{ alignSelf: "center", fontSize: "13px", color: "#666", fontWeight: 600 }}>Page {page + 1} of {totalPages}</span>
          <button className="inv-btn-primary-sm" disabled={page + 1 >= totalPages} onClick={() => setPage(page + 1)}>Next</button>
        </div>
      )}
    </div>
  );
