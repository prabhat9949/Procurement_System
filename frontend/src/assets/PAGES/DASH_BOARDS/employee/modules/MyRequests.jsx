...
const MyRequests = ({ onNavigate, onSelectTracking }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!me?.employeeId) {
        throw new Error("Your account is not linked to an employee record, so your requests cannot be loaded.");
      }
      const query = `?requesterId=${me.employeeId}&page=0&size=50&sort=createdAt&direction=desc`;
