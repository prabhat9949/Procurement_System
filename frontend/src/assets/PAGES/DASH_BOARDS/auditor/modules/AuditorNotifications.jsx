...
import { Bell, CheckCircle2, Loader2, WifiOff, ShieldCheck } from "lucide-react";
    ...
    <div style={{ padding: "20px" }}>
      <div className="aud-page-header">
        <div>
          <h1 className="aud-page-title" style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <Bell color="#dc2626" /> Notifications
          </h1>
          <p className="aud-page-subtitle">Workflow events and audit updates — live from the database.</p>
        </div>
        <button className="aud-btn-primary-sm" onClick={markAllRead} disabled={!notifs.some((n) => !n.isRead)}>
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: "12px", alignItems: "center" }}>
                  ...
                  {!n.isRead && <span style={{ fontSize: "10px", fontWeight: "800", color: "#dc2626", background: "#fef2f2", padding: "2px 8px", borderRadius: "10px" }}>NEW</span>}
                </div>
      ...
      {totalPages > 1 && (
        <div style={{ display: "flex", justifyContent: "center", gap: "10px", marginTop: "20px" }}>
          <button className="aud-btn-primary-sm" style={{ background: "#f8f9fb", color: "#111", border: "1px solid #d9d9d9" }} disabled={page === 0} onClick={() => setPage(page - 1)}>Previous</button>
          <span style={{ alignSelf: "center", fontSize: "13px", color: "#666", fontWeight: 600 }}>Page {page + 1} of {totalPages}</span>
          <button className="aud-btn-primary-sm" disabled={page + 1 >= totalPages} onClick={() => setPage(page + 1)}>Next</button>
        </div>
      )}
    </div>
  );
