...
const toTeamRequest = (request) => ({
  ...request,
  id: request.requestNumber || String(request.id),
  requester: request.requesterName || request.requester?.fullName || "Unavailable",
  role: request.requesterDesignation || "—",
  empId: request.requesterEmployeeCode || "—",
  dept: request.departmentName || "—",
  costCenter: request.costCenterName || "—",
  product: request.purpose || "—",
  category: request.categoryName || "—",
  vendor: request.preferredVendorName || "—",
  cost: formatAmount(request.estimatedAmount),
  rawCost: Number(request.estimatedAmount || 0),
  priority: request.priority || "NORMAL",
  status: (request.status || "DRAFT").toLowerCase(),
  date: request.createdAt ? new Date(request.createdAt).toLocaleDateString("en-IN") : "—",
  time: request.createdAt ? new Date(request.createdAt).toLocaleTimeString("en-IN") : "—",
  justification: request.purpose || request.remarks || "—",
  attachments: [],
  managerDecision: null,
});
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!profile.departmentId) throw new Error("Your account has no department scope configured.");
        const data = await getPurchaseRequests({ departmentId: profile.departmentId });
        if (active) setTeamRequests((Array.isArray(data) ? data : data?.content || []).map(toTeamRequest));
      } catch (error) {
        if (active) setLoadError(error.message || "Unable to load department requisitions.");
      } finally {
    ...
    <div className="dm-team-requisitions-container">
      ...
      <div className="dm-page-header">
        <div>
          ...
          <p className="dm-page-subtitle">Purchase requisitions in your backend-assigned department.</p>
        </div>
      ...
      <div className="dm-card">
        <div className="dm-table-container">
          <table className="dm-table">
            ...
            <tbody>
              {loading && <tr><td colSpan="9" style={{ textAlign: "center", padding: "32px" }}>Loading requisitions…</td></tr>}
              {!loading && loadError && <tr><td colSpan="9" style={{ textAlign: "center", padding: "32px", color: "#b91c1c" }}><AlertCircle size={16} /> {loadError}</td></tr>}
              {!loading && !loadError && filtered.length === 0 && <tr><td colSpan="9" style={{ textAlign: "center", padding: "32px" }}>No requisitions were found for your department.</td></tr>}
              {filtered.map((req) => (
                <tr key={req.id}>
