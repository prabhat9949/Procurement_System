...
const ApprovalQueue = ({ onTrackForm }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!me?.employeeId) {
        throw new Error("Your account is not linked to an employee record, so the approval queue cannot be loaded.");
      }
      const query = `?assignedEmployeeId=${me.employeeId}&status=PENDING&page=0&size=50&sort=assignedDate&direction=desc`;
  ...
  const submitDecision = async (e) => {
    ...
    try {
      ...
      setCompletedRequestId(targetTask.purchaseRequestId);
