  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [tasks, prs, prApproved, spend, prTrend] = await Promise.all([
        apiGet("/api/approval-tasks?page=0&size=20&sort=assignedDate&direction=desc").catch(() => null),
        apiGet("/api/purchase-requests?page=0&size=1").catch(() => null),
        apiGet("/api/purchase-requests?status=APPROVED&page=0&size=1").catch(() => null),
