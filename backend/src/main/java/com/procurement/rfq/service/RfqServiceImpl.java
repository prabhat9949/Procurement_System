...
@Transactional public RfqResponse generate(RfqRequest r){
    ...
    e=repo.save(e);
    final Rfq saved=e;
    for(PurchaseRequestLine pl:requestLines.findByPurchaseRequestId(pr.getId())){
        // For a partial fulfilment the RFQ must only cover the shortage: requested
        // minus the quantity already allocated internally (so we never externally
        // procure what is fulfilled from stock).
        java.math.BigDecimal qty = pl.getQuantity();
        if (pr.getStatus()==PurchaseRequestStatus.PARTIAL_FULFILMENT_PENDING && pl.getId()!=null){
            java.math.BigDecimal allocated = internalFulfilments.findByPurchaseRequestIdOrderByCreatedAtAsc(pr.getId()).stream()
                    .filter(f -> f.getPurchaseRequestLine()!=null && f.getPurchaseRequestLine().getId().equals(pl.getId()))
                    .filter(f -> f.getAllocatedQuantity()!=null)
                    .map(f -> f.getAllocatedQuantity())
                    .reduce(java.math.BigDecimal.ZERO, java.math.BigDecimal::add);
            qty = qty.subtract(allocated).max(java.math.BigDecimal.ZERO);
        }
        lines.save(RfqLine.builder().rfq(saved).product(pl.getProduct()).quantity(qty).requiredDate(pr.getRequiredDate()).estimatedUnitPrice(pl.getUnitPrice()).remarks(pl.getRemarks()).build());
    }
    ...
    vendors.findAll().stream()
            ...
            .forEach(v -> {
                if (!rfqVendors.existsByRfqIdAndVendorId(saved.getId(), v.getId())) {
                    rfqVendors.save(RfqVendor.builder().rfq(saved).vendor(v)
