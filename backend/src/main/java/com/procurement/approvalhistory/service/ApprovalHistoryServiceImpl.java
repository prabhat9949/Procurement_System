...
@Service public class ApprovalHistoryServiceImpl implements ApprovalHistoryService{
    ...
    @Transactional(readOnly=true)public PageResponse<ApprovalHistoryResponse>search(Long requestId,Long performedById,Pageable p){
        if(isEmployeeOnly()){
        ...
        Specification<ApprovalHistory>s=(r,q,b)->{
            var c=b.conjunction();
            if(requestId!=null)c=b.and(c,b.equal(r.get("purchaseRequest").get("id"),requestId));
            if(performedById!=null)c=b.and(c,b.equal(r.get("performedBy").get("id"),performedById));
            return c;
        };
